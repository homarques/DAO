...
plotSynth <- function () {
	...
	p = ggplot(data=df, aes(x=Dimensionality, y=AUCROC, group=Algorithms, color=Algorithms, shape = Algorithms)) + geom_line() + geom_point(size = 8) + geom_segment(aes(x = 8, y = -Inf, xend = 8, yend = 1), linetype='C9',colour='black' ) +
	...
	ggsave("synthetic.png", width=28, height = 18)
	print(p)
	dev.off()
...
summaryResultsSyntheticDatasets <- function () {
	...
	results_table = cbind(c(paste('Intrinsic Dimension = ', seq(2,32,2)), 'Overall'), results_table)
	colnames(results_table) = names
...
plot_R_MoransI <- function () {
	...
	p = ggplot(tb, aes(x = Moran, y = R, color = diff)) + geom_point(cex = 3.5) + ylab(R_title) + xlab(morans_title) + 
	...
	p = ggplot(tb, aes(x = Moran, y = R, color = (diff))) + geom_point(cex = 3.5) + 
	...
	p = ggplot(tb, aes(x = Moran, y = R, color = (diff), shape = orc$Algorithms)) + geom_point(cex = 3.5) + labs(shape="") + ylab(R_title) + xlab(morans_title) + 
	scale_colour_gradientn(values=c(0, 0.25, 0.35, 0.4999, 0.5001, 0.65, 0.75, 1), colours = (brewer.pal(name='RdBu', n = 11)[c(1, 2, 3, 8, 9, 10, 11)]),limit = limit, name = "    ROC AUC\n(DAO - Oracle)") +
...
if(as.character(args[1]) == "summaryRealDatasets"){
	summaryRealDatasets()
}
if(as.character(args[1]) == "summaryResultsSyntheticDatasets"){
	plotSynth()
	summaryResultsSyntheticDatasets()
}
if(as.character(args[1]) == "lrSyntheticDatasets"){
	lrSyntheticDatasets()
}
if(as.character(args[1]) == "printRuntime"){
	printRuntime()
}
if(as.character(args[1]) == "lrRealDatasets"){
	lrRealDatasets()
}
if(as.character(args[1]) == "plot_R_MoransI"){
	plot_R_MoransI()
}
if(as.character(args[1]) == "plotCDRealDatasets"){
	plotCDRealDatasets()
}
